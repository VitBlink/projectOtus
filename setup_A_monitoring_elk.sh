...
if dpkg -l | grep -q "^ii  elasticsearch"; then
   ...
   [ -f "${ES_CONF}.orig" ] || cp "$ES_CONF" "${ES_CONF}.orig"
   ...
   set_yaml "cluster.name"                      "test-cluster"  "$ES_CONF"
   set_yaml "node.name"                         "node-1"        "$ES_CONF"
   set_yaml "network.host"                      "localhost"     "$ES_CONF"
   set_yaml "http.port"                         "9200"          "$ES_CONF"
   set_yaml "discovery.type"                    "single-node"   "$ES_CONF"
   set_yaml "xpack.security.enabled"            "false"         "$ES_CONF"
   set_yaml "xpack.security.enrollment.enabled" "false"         "$ES_CONF"
   set_yaml "bootstrap.memory_lock"             "false"         "$ES_CONF"
   set_yaml "path.data"                         "/var/lib/elasticsearch" "$ES_CONF"
   set_yaml "path.logs"                         "/var/log/elasticsearch" "$ES_CONF"

   echo "elasticsearch.yml обновлён точечно"
